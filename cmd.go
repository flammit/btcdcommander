...
// NotifyNewTransactions requests all transaction notifications
func (c *Commander) NotifyNewTransactions(verbose bool) *btcjson.Error {
	rpc := c.currentRpcConn()

	// error not possible with single argument
	cmd, _ := btcws.NewNotifyNewTransactionsCmd(<-c.newJSONID, verbose)
	response := <-rpc.sendRequest(cmd, nil)
	return response.err
}
